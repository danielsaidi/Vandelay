...
public class ExportAlertController: UIAlertController {
    ...
    public func addDataExporter(_ exporter: DataExporter, withTitle title: String) {
    ...
    public func addStringExporter(_ exporter: StringExporter, withTitle title: String) {
        let action = UIAlertAction(title: title, style: .default) { action in
            self.delegate?.alert(self, didPick: exporter)
        }
        addAction(action)
